...
export type JaroWinklerOptions = {
  prefixScale?: number; // Default: 0.1
...
export const levDist = (a: string, b: string): number => {
    ...
    for (let j = 0; j <= lb; j++) {
      if (i == 0) {
        ...
        matrix[i][j] =
          Math.min(
            matrix[i][j - 1],
            Math.max(matrix[i - 1][j], matrix[i - 1][j - 1]),
          ) + 1;
      }
    }
  }
...
export const levDistNormalized = (a: string, b: string): number => {
  const la = a.length;
  const lb = b.length;
...
export const levenshteinSimilarity = (
  a: string,
  b: string,
  options: LevenshteinSimiliarityOptions = {},
): number => {
...
export const naiveStringSimilarity = (a: string, b: string): number => {
  if (a === b) return 1.0;
  let score: number = 0;
...
export const cosineStringSimilarity = (a: string, b: string, options: CosineStringSimilarityOptions = {}) => {
  ...
  if (vecsA.length <= 0 || vecsB.length <= 0) return -1;
