...
import { triangleBarycentric } from '../mesh/triangle';
import { physicsBodyPointLocalToGlobal } from '../physics/body';
import { getRelativeVelocity } from '../physics/utils';
import { emptyMinkowskiTriangle, getMinkowskiSupportPair, } from '../utils/minkowski';
import { VEC3, VEC31, VEC4, samedir, vector3_add, vector3_cross, vector3_dot, vector3_mag, vector3_scale, vector3_sub, vector3_tangents_fast, vector3_unit, } from '../vector';
export const defaultEPAConfig = {
    threshold: 0.00019,
    maxIterations: 1000,
};
...
const extractPoint = (tri, is_a) => {
    ...
    const bary = triangleBarycentric(triangle, tri.normal.scale(distanceFromOrigin));
    ...
    point = vector3_add(point, is_a ? tri.v1.a.furthest.scale(bary.x) : tri.v1.b.furthest.scale(bary.x));
    point = vector3_add(point, is_a ? tri.v2.a.furthest.scale(bary.y) : tri.v2.b.furthest.scale(bary.y));
    point = vector3_add(point, is_a ? tri.v3.a.furthest.scale(bary.z) : tri.v3.b.furthest.scale(bary.z));
    return point;
};
    ...
    */
    ...
    for (let iter = 0; iter < cfg.maxIterations; iter += 1) {
        ...
        if (next_diff <= cfg.threshold && min_distance < 1600.0) {
            ...
            let velTangent = vector3_sub(sepvel, vector3_scale(min_normal, velAlongNormal));
            let t1 = velTangent;
            let t2 = t1;
